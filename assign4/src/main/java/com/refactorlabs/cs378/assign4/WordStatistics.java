...
public class WordStatistics  {
    ...
    public static class MapClass extends Mapper<LongWritable, Text, Text, WordStatisticsWritable> {
         ...
         * simply reset its value as data for each word output.
         */
        private WordStatisticsWritable valueArray = new WordStatisticsWritable();
    ...
    public static class ReduceClass extends Reducer<Text, WordStatisticsWritable, Text, WordStatisticsWritable> {
        ...
        public void reduce(Text key, Iterable<WordStatisticsWritable> values, Context context)
                throws IOException, InterruptedException {
            WordStatisticsWritable outputValue = new WordStatisticsWritable();
            long[] sum = outputValue.sumLong(values);
            ...
            Writable[] writableArray = new Writable[5];
            double mean = (double)sum[1] / sum[0];

            writableArray[0] = new DoubleWritable(sum[0]);
            writableArray[1] = new LongWritable(sum[1]);
            writableArray[2] = new LongWritable(sum[2]);
            writableArray[3] = new DoubleWritable(mean);
            writableArray[4] = new DoubleWritable((double)sum[2] / sum[0] - mean * mean);
    ...
    public static void main(String[] args) throws Exception {
        ...
        job.setJarByClass(WordStatisticsWritable.class);
