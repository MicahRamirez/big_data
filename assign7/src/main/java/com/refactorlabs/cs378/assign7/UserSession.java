...
public class UserSession extends Configured implements Tool {
    ...
    public static class MapClass extends Mapper<LongWritable, Text, Text, AvroValue<Session>> {
        ...
        private static final String[] subTypeSpace = {"CONTACT FORM TYPE", "ALTERNATIVE", "CONTACT BANNER", "CONTACT BUTTON","DEALER PHONE", "FEATURES", "GET DIRECTIONS","SHOW MORE BADGES", "TEST DRIVE LINK",
                "VEHICLE HISTORY","FORM ERROR", "FORM SUCCESS", "CONTACT FORM", "MARKET REPORT","BADGE DETAIL", "PHOTO MODAL","BADGES"};
        ...
        public void map(LongWritable key, Text value, Context context)
            ...
            int index = 1;
            //1
            evenTypeAndSub(event, input[index]);
            index++;
            //2
        ...
        private static void evenTypeAndSub(Event event, String input){
            if(input.equalsIgnoreCase("VISIT Market Report Listing")){
                event.setEventType(EventType.VISIT);
                event.setEventSubtype(EventSubtype.MARKET_REPORT);
            }else{
                String[] splitPhrase = StringUtils.split(input, " ", 1);
                if(splitPhrase.length == 1){
                    splitPhrase = StringUtils.split(input, " ", 2);
                }
                ...
                if(splitPhrase.length == 1){
                    System.out.println(splitPhrase[0]);
                }

                for(int j = 0; j < subType.length; j++){
                    if(splitPhrase.length > 1 && subTypeSpace[j].toString().equalsIgnoreCase(splitPhrase[1])) {
